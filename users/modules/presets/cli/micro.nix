...
let
  baseEnabled = config.profiles.base.enable;
  microEnabled = config.presets.micro.enable;
in
{
  _file = ./micro.nix;

  options = {
    presets.micro.enable = lib.mkEnableOption "Micro editor";
  };

  config = lib.mkIf (baseEnabled || microEnabled) {
    ...
    xdg.configFile = {
      "micro/bindings.json".source = ./micro-bindings.json;
